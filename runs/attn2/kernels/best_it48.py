...
@ct.kernel
def O_kernel(P, V, O, DD: ConstInt, J: ConstInt, TS_i: ConstInt, TS_dd: ConstInt, TS_j: ConstInt):
    i = ct.bid(0)
    dd = ct.bid(1)
    acc1 = ct.zeros((TS_i, TS_dd), dtype=O.dtype)
    for j in range(ct.cdiv(J, TS_j)):
        t2 = ct.load(P, (i, j), (TS_i, TS_j))
        t3 = ct.load(V, (j, 0), (TS_j, DD))
        acc1 = ct.mma(t2, t3, acc1)
    ct.store(O, (i, 0), acc1)
...
def fn(KT, Q, V):
    ...
    ct.launch(stream, grid, O_kernel, (P, V, O, 64, 512, TILE_O[0], TILE_O[1], RTILE_O))
    return O
