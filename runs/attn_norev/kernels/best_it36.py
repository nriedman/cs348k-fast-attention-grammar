...
@ct.kernel
def e_kernel(mx, Ss, e, TS_i: ConstInt, TS_j: ConstInt):
    i = ct.bid(0)
    j = ct.bid(1)
    t1 = ct.load(mx, (i, 0), (TS_i, 1))
    t2 = ct.load(Ss, (i, j), (TS_i, TS_j))
    t3 = (t2 - t1)
    t4 = ct.exp(t3)
    ct.store(e, (i, j), t4)
...
def fn(KT, Q, V):
    ...
    ct.launch(stream, grid, e_kernel, (mx, Ss, e, TILE_e[0], TILE_e[1]))
