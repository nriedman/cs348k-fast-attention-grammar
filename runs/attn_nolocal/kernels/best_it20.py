...
def Ss_kernel(Q, KT, Ss, DD: ConstInt, TS_i: ConstInt, TS_j: ConstInt, TS_dd: ConstInt):
    ...
    acc3 = ct.zeros((TS_i, TS_j), dtype=Ss.dtype)
    for dd in range(ct.cdiv(DD, TS_dd)):
        t4 = ct.extract(t1, (0, dd), (TS_i, TS_dd))
        t5 = ct.extract(t2, (dd, 0), (TS_dd, TS_j))
...
@ct.kernel
def P_kernel(sm, e, P, TS_i: ConstInt, TS_j: ConstInt):
    i = ct.bid(0)
    j = ct.bid(1)
    t1 = ct.load(sm, (i, 0), (TS_i, 1))
    t2 = ct.load(e, (i, j), (TS_i, TS_j))
    t3 = (t2 / t1)
    ct.store(P, (i, j), t3)
...
def fn(KT, Q, V):
    ...
    ct.launch(stream, grid, P_kernel, (sm, e, P, TILE_P[0], TILE_P[1]))
